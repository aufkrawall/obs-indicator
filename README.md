...
You need to enable WebSocket server in obs (Tools -> WebSocket Server Settings), the tool is retrieving obs status via that function. Supports password authentication, stores password in Base64 format in config and should not unnecessarily keep unencrypted password in RAM. Might be AI generated.
